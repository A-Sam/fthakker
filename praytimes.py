...
class PrayTimes():
        ...
        for method, config in self.methods.items():
            for name, value in self.defaultParams.items():
                if not name in config[
                        'params'] or config['params'][name] is None:
    ...
    def getTimes(self, date, coords, timezone, dst=0, format=None):
        ...
        self.jDate = self.julian(date[0], date[1],
                                 date[2]) - self.lng / (15 * 24.0)
        print("jDate:")
    ...
    def getFormattedTime(self, time, format, suffixes=None):
        ...
        formattedTime = "%02d:%02d" % (
            hours, minutes) if format == "24h" else "%d:%02d" % (
    ...
    def julian(self, year, month, day):
        ...
        return math.floor(365.25 * (year + 4716)) + math.floor(
    ...
    def computePrayerTimes(self, times):
        ...
        imsak = self.sunAngleTime(self.eval(params['imsak']), times['imsak'],
                                  'ccw')
        fajr = self.sunAngleTime(self.eval(params['fajr']), times['fajr'],
                                 'ccw')
        sunrise = self.sunAngleTime(self.riseSetAngle(self.elv),
                                    times['sunrise'], 'ccw')
        dhuhr = self.midDay(times['dhuhr'])
        asr = self.asrTime(self.asrFactor(params['asr']), times['asr'])
        sunset = self.sunAngleTime(self.riseSetAngle(self.elv),
                                   times['sunset'])
        maghrib = self.sunAngleTime(self.eval(params['maghrib']),
                                    times['maghrib'])
    ...
    def computeTimes(self):
        ...
        if self.settings['midnight'] == 'Jafari':
            times['midnight'] = times['sunset'] + self.timeDiff(
                times['sunset'], times['fajr']) / 2
        else:
            times['midnight'] = times['sunset'] + self.timeDiff(
                times['sunset'], times['sunrise']) / 2
    ...
    def adjustTimes(self, times):
        ...
        if self.isMin(params['maghrib']):
            times['maghrib'] = times['sunset'] - self.eval(
                params['maghrib']) / 60.0
    ...
    def asrFactor(self, asrParam):
        ...
        return methods[asrParam] if asrParam in methods else self.eval(
            asrParam)
    ...
    def adjustHighLats(self, times):
        params = self.settings
        nightTime = self.timeDiff(times['sunset'],
                                  times['sunrise'])  # sunset to sunrise
        times['imsak'] = self.adjustHLTime(times['imsak'], times['sunrise'],
                                           self.eval(params['imsak']),
                                           nightTime, 'ccw')
        times['fajr'] = self.adjustHLTime(times['fajr'], times['sunrise'],
                                          self.eval(params['fajr']), nightTime,
                                          'ccw')
        times['isha'] = self.adjustHLTime(times['isha'], times['sunset'],
                                          self.eval(params['isha']), nightTime)
        times['maghrib'] = self.adjustHLTime(times['maghrib'], times['sunset'],
...
if __name__ == "__main__":
    ...
    times = prayTimes.getTimes(date.today(), (48.092724, 11.502992), 2)
    for i in [
